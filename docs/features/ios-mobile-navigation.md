...
- The app fetches a validated public content snapshot from `/api/mobile-content` at runtime. Loading, unavailable, empty, no-match, and missing-detail states are explicit.
...
- Given runtime loading fails, when the unavailable state appears and the resident retries, then the app makes a new request and recovers when content is available.
...
- Loading, retry, empty, no-match, and missing-detail handling.
