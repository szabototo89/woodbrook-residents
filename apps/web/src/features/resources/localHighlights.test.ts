...
describe('local highlights', () => {
  it('summarises the next featured collection dates and qualification', () => {
  ...
  it('supports featured information without collection dates', () => {
    const highlights = getLocalHighlights(
      [
        resource({
          category: 'transport',
  ...
  it('omits ordinary resources and expired schedules', () => {
    expect(
      getLocalHighlights(
        [
          ...
          resource({
            documentId: 'expired',
            collectionDates: [
              { id: 1, date: '2026-09-01', stream: 'recycling' },
            ],
          }),
        ],
        '2026-09-11',
