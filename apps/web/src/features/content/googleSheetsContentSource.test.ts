...
test('parseGoogleSheetsContent filters drafts and maps a published project into the domain model', () => {
  const sheets = {
    Updates: [[]],
    Events: [[]],
    Projects: [
...
test('parseGoogleSheetsContent maps local information fields into public resource details', () => {
  const headers = [
    'record_id',
    'slug',
    'publish',
...
test('parseGoogleSheetsContent reports the tab, row, and field for invalid published content', () => {
  expect(() =>
    parseGoogleSheetsContent({
      Updates: [[]],
      Events: [[]],
...
test('parseGoogleSheetsContent requires build-only service-account credentials before reading', async () => {
...
test('parseGoogleSheetsContent reads all content tabs in one authenticated read-only batch request', async () => {
...
test('parseGoogleSheetsContent accepts every deliberately configured spreadsheet taxonomy family', () => {
  const updateHeaders = [
    'record_id',
    'slug',
    'publish',
...
test('parseGoogleSheetsContent maps Dublin spreadsheet date-times and consultation relations', () => {
...
test('parseGoogleSheetsContent uses sort order first and the content date second', () => {
  const headers = [
    'record_id',
    'slug',
    'publish',
...
test('parseGoogleSheetsContent validates editorial taxonomy fields even when the domain does not display them', () => {
  expect(() =>
    parseGoogleSheetsContent({
      Updates: [[]],
      Events: [
...
test('parseGoogleSheetsContent enforces required spreadsheet fields before canonical validation', () => {
  expect(() =>
    parseGoogleSheetsContent({
      Updates: [[]],
      Events: [[]],
...
test('parseGoogleSheetsContent field handling rejects non-text values in text fields', () => {
  expect(() => parseUpdates([updateRow({ 6: true })])).toThrow(
    'Updates row 2 field "title": must be text.',
  );
});

test('parseGoogleSheetsContent field handling rejects blank values in required text fields', () => {
  expect(() => parseUpdates([updateRow({ 6: '   ' })])).toThrow(
    'Updates row 2 field "title": is required.',
  );
});

test('parseGoogleSheetsContent field handling treats blank optional text as missing', () => {
  const snapshot = parseUpdates([updateRow({ 8: '   ' })]);

  expect(snapshot.updates[0]?.body).toBe('');
});

test('parseGoogleSheetsContent field handling rejects a row without a publish decision', () => {
  expect(() => parseUpdates([['u1', 'u1']])).toThrow(
    'Updates row 2 field "publish": is required.',
  );
});

test('parseGoogleSheetsContent field handling accepts string booleans for publish', () => {
  const snapshot = parseUpdates([updateRow({ 2: 'TRUE' })]);

  expect(snapshot.updates).toHaveLength(1);
});

test('parseGoogleSheetsContent field handling rejects invalid booleans', () => {
  expect(() => parseUpdates([updateRow({ 2: 'yes' })])).toThrow(
    'Updates row 2 field "publish": must be TRUE or FALSE.',
  );
});

test('parseGoogleSheetsContent field handling rejects invalid dates', () => {
...
test('parseGoogleSheetsContent field handling rejects missing required dates', () => {
  expect(() => parseUpdates([updateRow({ 5: '' })])).toThrow(
    'Updates row 2 field "published_on": is required.',
  );
});

test('parseGoogleSheetsContent field handling ignores non-string header cells', () => {
  const snapshot = parseGoogleSheetsContent({
    Updates: [
      [...updateHeaders, ''],
...
test('parseGoogleSheetsContent project handling rejects non-numeric coordinates', () => {
  expect(() => parseProjects([projectRow({ 11: 'north' })])).toThrow(
    'Projects row 2 field "latitude": must be a number.',
  );
});

test('parseGoogleSheetsContent project handling accepts numeric text for coordinates', () => {
  const snapshot = parseProjects([projectRow({ 11: '53.21' })]);

  expect(snapshot.projects).toHaveLength(1);
});

test('parseGoogleSheetsContent project handling rejects slugs outside lowercase kebab-case', () => {
  expect(() => parseProjects([projectRow({ 1: 'Bad Slug!' })])).toThrow(
    'Projects row 2 field "slug": must be lowercase kebab-case.',
  );
});

test('parseGoogleSheetsContent project handling requires the source URL', () => {
  expect(() => parseProjects([projectRow({ 17: '' })])).toThrow(
    'Projects row 2 field "source_url": is required.',
  );
});

test('parseGoogleSheetsContent project handling rejects invalid source URLs', () => {
  expect(() => parseProjects([projectRow({ 17: 'not a url' })])).toThrow(
    'Projects row 2 field "source_url": must be a valid public URL.',
  );
});

test('parseGoogleSheetsContent project handling rejects source URLs without a public protocol', () => {
  expect(() =>
    parseProjects([projectRow({ 17: 'ftp://example.com/p' })]),
  ).toThrow('Projects row 2 field "source_url": must be a valid public URL.');
});

test('parseGoogleSheetsContent project handling sorts equal orders by review date and defaults missing bodies', () => {
...
test('parseGoogleSheetsContent event handling rejects invalid start times', () => {
  expect(() => parseEvents([eventRow({ 8: 'sometime' })])).toThrow(
    'Events row 2 field "start_at": must be a real spreadsheet date-time.',
  );
});

test('parseGoogleSheetsContent event handling rejects missing required start times', () => {
  expect(() => parseEvents([eventRow({ 8: '' })])).toThrow(
    'Events row 2 field "start_at": is required.',
  );
});

test('parseGoogleSheetsContent event handling accepts ISO string date-times', () => {
...
test('parseGoogleSheetsContent event handling books through the organiser when registration is missing', () => {
  const snapshot = parseEvents([
    eventRow({ 16: 'https://example.com/organiser' }),
  ]);

  expect(snapshot.events[0]?.bookingUrl).toBe('https://example.com/organiser');
});

test('parseGoogleSheetsContent event handling sorts equal orders by start time', () => {
  const snapshot = parseEvents([
...
test('parseGoogleSheetsContent event handling requires a location name or address', () => {
...
test('parseGoogleSheetsContent consultation handling sorts equal orders by closing date, then opening date', () => {
  const snapshot = parseGoogleSheetsContent({
    Updates: [[]],
    Events: [[]],
    Projects: [[]],
...
test('parseGoogleSheetsContent resource handling falls back to the address when no card detail is set', () => {
...
test('parseGoogleSheetsContent resource handling skips duplicate resource details', () => {
...
test('parseGoogleSheetsContent resource handling keeps same-label details with different values', () => {
  const snapshot = parseResources([
    resourceRow({
      14: 'Opening hours',
      15: 'See website',
...
test('parseGoogleSheetsContent resource handling orders shared collection days by stream', () => {
  const snapshot = parseResources([
...
test('parseGoogleSheetsContent resource handling defaults display order to zero', () => {
  const snapshot = parseResources([resourceRow({ 25: '' })]);

  expect(snapshot.resources[0]?.displayOrder).toBe(0);
});

test('parseGoogleSheetsContent resource handling sorts equal orders by title', () => {
  const snapshot = parseResources([
...
test('parseGoogleSheetsContent resource handling rejects invalid collection dates', () => {
...
test('parseGoogleSheetsContent resource handling rejects protocol-relative document URLs', () => {
  expect(() =>
...
test('GoogleSheetsContentSource credential handling requires the private key when the email is configured', async () => {
  const source = new GoogleSheetsContentSource({
    spreadsheetId: 'sheet-id',
    serviceAccountEmail: 'reader@example.test',
    serviceAccountPrivateKey: '',
...
test('GoogleSheetsContentSource credential handling rejects a batch response with missing ranges', async () => {
...
test('GoogleSheetsContentSource credential handling treats ranges without values as empty', async () => {
  googleAuthMock.request.mockResolvedValue({
    data: { valueRanges: [{}, {}, {}, {}, {}] },
  });
  const source = new GoogleSheetsContentSource({
