...
function pagespeedPayload() {
  return {
    lighthouseResult: {
      requestedUrl: 'https://www.julietrosebeauty.com/',
      finalUrl: 'https://www.julietrosebeauty.com/',
      categories: {
...
test('config defaults to production routes and reads the API key from env', () => {
  ...
  expect(config.siteUrl).toBe('https://www.julietrosebeauty.com');
...
test('config accepts a route filter and strategy', () => {
  const config = resolvePagespeedConfig(['--route=/', '--strategy=desktop'], {
    GOOGLE_PAGESPEED_API_KEY: 'test-key',
  });
  expect(config.routes).toEqual(['/']);
...
test('pagespeed URL targets the v5 API with performance and seo categories', () => {
  ...
  expect(url).toContain(
    `url=${encodeURIComponent('https://www.julietrosebeauty.com/')}`,
  );
...
test('summary line passes a good route and flags a slow one', () => {
  ...
  expect(formatRouteSummary('/', good)).toContain('PASS');
  ...
  const line = formatRouteSummary('/', slow);
